...
function Nav(textInInput) {
  ...
  let url = `https://collectionapi.metmuseum.org/public/collection/v1/search?isHighlight=true&isImage=true&q=sunflowers`;
  if (textInput.length == 2) {
    console.log(
      `empty as length = ${textInput.length}, so url=${url} and the textInput is ${textInput}`
    );
  } else {
    url = `https://collectionapi.metmuseum.org/public/collection/v1/search?isHighlight=true&isImage=true&q=mohamed`;
